...
export const JsonTreeNode = memo(function JsonTreeNode({ value, depth }: { value: unknown; depth: number }): React.ReactElement {
...
const JsonTreeObj = memo(function JsonTreeObj({ obj, depth }: { obj: Record<string, unknown>; depth: number }) {
...
const JsonTreeArr = memo(function JsonTreeArr({ arr, depth }: { arr: unknown[]; depth: number }) {
  const [collapsed, setCollapsed] = useState(depth >= 2)
